...
test('Clearing command buffer starts with \\nN\\n', () => {
    expect(new LabelEpl(0, 0, 0, 0).clearCommandBuffer().commandBuffer).toBe("\nN\n");
});
...
test('Setting line spacing sets line spacing', () => {
    expect(new LabelEpl(0, 0, 0, 0).setLineSpacing(4).lineSpacing).toBe(4);
});

test('Setting font size sets font size', () => {
    expect(new LabelEpl(0, 0, 0, 0).setFont(4).font).toBe(4);
});

test('Setting default font size sets font size', () => {
    expect(new LabelEpl(0, 0, 0, 0).setFont().font).toBe(1);
});

test('Setting font size out of bounds sets font size', () => {
    expect(new LabelEpl(0, 0, 0, 0).setFont(-1).font).toBe(1);
});

test('Setting and clearing offsets works', () => {
    const label = new LabelEpl(0, 0, 0, 0);
...
test('Adding text adds text command to buffer', () => {
    const label = new LabelEpl(203, 203, 203, 0);
